@module {plugin} can-map-attributes attributes
@parent can-legacy
@plugin can/map/attributes
@test src/test/test.html
@download http://donejs.com/can/dist/can.map.attributes.js
@group can-map-attributes.static 0 static
@group can-map-attributes.prototype 1 prototype
...
@demo ../src/demo/attributes.html

### Reference types

Types listed in `attributes` can also be a functions, such as the `model` or
`models` methods of a [can-model Model]. When data of this kind of type is set, this
...
If the value to convert is an array it will be converted into its `[can-list Map.List]` or `[can-model.List Model.List]` (using `[can-model.models Model.models]`) representation:
...
@demo ../src/demo/attributes-assocations.html
